...
on the type of target interconnect they model. A pipe is intialized with tokens
...
\emph{Deadlock} occurs if no model can advance (not all of its input tokes are
...
source into the channel, preserving the RTL behavior of the intial SDF but
...
\begin{figure}
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        ...
        \caption{Solution A, use a pipe. Moving registers into the channel adds an intial token. While
...
bound a single AXI4 master port.\footnote{We recognize this is an uneeded serialization but have only had to support single-channel FPGA-host memory systems to date.}
...
facilitated with simulation MMIO that must be implemented with a host-plaform
specific driver. This simulation MMIO driver supports read and write requests
made by the libaries by tunneling these requests over the interconnect available
on the host.
